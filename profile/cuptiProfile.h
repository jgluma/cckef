 ...
 * @brief Original file downloaded from https://github.com/srvm/cupti_profiler
 * @version 0.1
 * @date 2021-03-23
 * 
...
namespace detail {
  ...
  struct pass_data_t {
    ...
    std::vector<uint64_t> event_instances;
  };
  ...
  struct kernel_data_t {
    ...
    std::vector<uint64_t> m_event_instances;
    ...
    void stop() {
      for(auto &k: m_kernel_data) {
        ...
        for(int i = 0; i < m_metric_passes; ++i) {
          ...
          for (auto ii = data[i].event_instances.begin(); ii != data[i].event_instances.end(); ++ii)
          {
              k.second.m_event_instances.push_back(*ii);
          }
        }
...
std::vector<std::string> get_kernel_names();
    ...
    std::vector<uint64_t> get_event_instances(const char *kernel_name) {
        return m_kernel_data[kernel_name].m_event_instances;
    }
  ...
  private:
    ...
    // array of event instances from current event and kernel
    std::vector<uint64_t> m_event_instances;
